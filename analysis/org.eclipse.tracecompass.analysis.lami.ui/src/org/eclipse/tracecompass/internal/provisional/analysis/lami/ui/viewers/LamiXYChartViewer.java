...
public abstract class LamiXYChartViewer extends TmfViewer implements ILamiViewer {
    ...
    /**
     * Create a tool bar on top right of the chart. Contains actions:
     *  - dispose of the current view also know as "Close the chart"
     *
     * This tool bar should only appear when the mouse enter the composite.
     *
     * @return the tool bar
     */
    protected ToolBar createChartToolBar() {
        ...
        /* Visibility toogle listener */
        ToolBarVisibilityToggleListener toolBarVisibilityToggleListener = new ToolBarVisibilityToggleListener();
        getChart().addListener(SWT.MouseEnter, toolBarVisibilityToggleListener);
        getChart().addListener(SWT.MouseExit, toolBarVisibilityToggleListener);
        for (Control control : getChart().getChildren()) {
            control.addListener(SWT.MouseEnter, toolBarVisibilityToggleListener);
            control.addListener(SWT.MouseExit, toolBarVisibilityToggleListener);
        }
    ...
    private final class ToolBarVisibilityToggleListener implements Listener {
        @Override
        public void handleEvent(@Nullable Event event) {
            getToolBar().setVisible(!getToolBar().getVisible());
        }
    }
}
